class ActionTurnOnWeaponLeftLight: ActionSingleUseBase {
	ItemBase m_Laser;
	void ActionTurnOnWeaponLeftLight() {
	...
	override typename GetInputType() return A6LightLeftActionInput;
	override bool HasTarget() {
		return false;
	}
	override bool ActionCondition( PlayerBase player, ActionTarget target, ItemBase item ) {
		if ( item.IsInherited(Weapon_Base) ) {
			m_Laser = ItemBase.Cast(item.FindAttachmentBySlotName("weaponLightLeft"));
		} else {
			return false;
		}	
...
class A6LightLeftActionInput : DefaultActionInput {
	void A6LightLeftActionInput(PlayerBase player) {
		SetInput("UAA6ToggleLightLeft");
