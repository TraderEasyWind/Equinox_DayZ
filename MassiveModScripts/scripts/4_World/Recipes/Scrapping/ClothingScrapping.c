...
{	
	override void Init()
	{
		...
		m_ResultReplacesIngredient[0] = 2;// value == -1 means do nothing; a value >= 0 means this result will transfer item propertiesvariables, attachments etc.. from an ingredient value
	}
...
class MassiveMod_LeatherSewingKit extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Craft Advanced Leather Sewing Kit";
		m_IsInstaRecipe = false;//should this recipe be performed instantly without animation
		m_AnimationLength = 1;//animation length in relative time units
		m_Specialty = -0.01;// value > 0 for roughness, value < 0 for precision
		...
		//result1
		AddResult("MassiveMod_ADVKit_LeatherSewing");//add results here
